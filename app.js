...
<section class="settings-section"><h3>🪄 CSS Preset Library</h3><p class="helper">模型输出仍经过 safe HTML sanitizer，不能注入 &lt;style&gt;。这里的 CSS 只来自你自己创建或导入的预设；默认只作用于 .tavern-story-content。</p><div class="toolbar"><button class="primary-btn" onclick="openCssPresetEditor()">＋ 新建 CSS</button><button class="ghost-btn" onclick="chooseCssImport()">⇧ 导入 .css</button><button class="danger-btn" onclick="emergencyDisableCss()">🚑 临时禁用自定义 CSS</button></div><div class="row-sub" style="margin:8px 0 12px">当前：${currentCssPreset()?escapeHtml(currentCssPreset().name):'未挂载'} · ${currentSessionData.customCssEnabled===false?'已禁用':'已启用'}</div><div class="toolbar">${currentSessionData.customCssId?'<button class="ghost-btn" onclick="unapplyCss()">取消应用</button>':''}${currentSessionData.customCssId&&currentSessionData.customCssEnabled===false?'<button class="ghost-btn" onclick="reenableCss()">重新启用</button>':''}</div><div id="css-preset-list" class="card-grid css-preset-grid" style="margin-top:12px"></div></section>
...
function compileCustomCss(preset){if(!preset?.css)return'';const scope=preset.scope==='global'?'#story-chat-box.tavern-story-scope':'.tavern-story-content';return scopeCssBlock(stripEmbeddedStyleTags(preset.css),scope)}
function applyCustomCss(){const tag=ensureCustomCssStyle();const preset=currentCssPreset();if(!currentSessionData||currentSessionData.customCssEnabled===false||!preset){tag.textContent='';return}tag.textContent=compileCustomCss(preset)}
function renderCssPresetLibrary(root){if(!root)return;const active=currentSessionData?.customCssId;root.innerHTML='';if(!cssPresets.length){root.innerHTML='<div class="row-sub">还没有 CSS Preset。可以新建或直接导入 .css。</div>';return}cssPresets.forEach(p=>{const mounted=p.id===active;const row=document.createElement('div');row.className=`library-card ${mounted?'mounted':''}`;row.innerHTML=`<div class="card-title"><span>${escapeHtml(p.name)}</span>${mounted?'<span class="tag">当前 Session</span>':''}</div><div class="card-desc">${p.scope==='global'?'剧情面板级 Scope':'消息内容级 Scope'} · ${String(p.css||'').length} 字符</div><div class="card-actions"><button class="${mounted?'primary-btn':'ghost-btn'}" onclick="applyCssPreset('${p.id}')">${mounted?'重新启用':'应用'}</button><button class="ghost-btn" onclick="openCssPresetEditor('${p.id}')">编辑</button><button class="ghost-btn" onclick="duplicateCssPreset('${p.id}')">复制</button><button class="ghost-btn" onclick="exportCssPreset('${p.id}')">导出</button></div>`;root.appendChild(row)})}
function openCssPresetEditor(id=null){const p=id?cssPresets.find(x=>x.id===id):{name:'',css:'',scope:'story'};editorState={type:'css-preset',id};qs('editor-title').textContent=id?`编辑 CSS · ${p?.name||''}`:'新建 CSS Preset';qs('editor-eyebrow').textContent='CSS PRESET';qs('editor-body').innerHTML=`<div class="form-grid"><div class="form-full"><label class="form-label">名称</label><input id="css-pre-name" class="field" value="${escapeHtml(p?.name||'')}"></div><div class="form-full"><label class="form-label">Scope</label><select id="css-pre-scope" class="field"><option value="story" ${p?.scope!=='global'?'selected':''}>消息内容级（最安全）</option><option value="global" ${p?.scope==='global'?'selected':''}>剧情面板级（不会碰设置中心）</option></select><div class="row-sub">即使选择“剧情面板级”，CSS 也只作用于剧情聊天区域，防止管理 UI 被误隐藏。</div></div><div class="form-full"><label class="form-label">CSS</label><textarea id="css-pre-code" class="field textarea tall css-code-editor" spellcheck="false" placeholder=".status-card { padding: 12px; border-radius: 16px; }">${escapeHtml(p?.css||'')}</textarea></div></div>`;setupEditorButtons(id?()=>deleteCssPreset(id):null,saveCssPresetEditor);qs('editor-modal').classList.remove('hidden')}
async function saveCssPresetEditor(){const body={name:qs('css-pre-name').value.trim()||'未命名 CSS',scope:qs('css-pre-scope').value==='global'?'global':'story',css:stripEmbeddedStyleTags(qs('css-pre-code').value)};const res=await fetchStory(editorState.id?`/css-presets/${editorState.id}`:'/css-presets',editorState.id?'PATCH':'POST',body);if(!res.success)return toast(res.msg||'保存 CSS 失败');const saved=res.data;closeEditor();await loadAll();if(currentSessionData?.customCssId===saved.id)applyCustomCss();openSettings('appearance');toast(`CSS Preset「${saved.name}」已保存`)}
async function applyCssPreset(id){if(!cssPresets.some(x=>x.id===id))return;await patchCurrent({customCssId:id,customCssEnabled:true},true);applyCustomCss();if(settingsTab==='appearance')renderAppearanceTab(qs('settings-content'));toast('CSS 已应用到当前 Session')}
async function unapplyCss(){await patchCurrent({customCssId:null,customCssEnabled:false},true);applyCustomCss();if(settingsTab==='appearance')renderAppearanceTab(qs('settings-content'));toast('当前 Session 已取消 CSS')}
async function emergencyDisableCss(){await patchCurrent({customCssEnabled:false},true);ensureCustomCssStyle().textContent='';if(settingsTab==='appearance')renderAppearanceTab(qs('settings-content'));toast('🚑 自定义 CSS 已临时禁用')}
async function reenableCss(){if(!currentSessionData?.customCssId)return toast('当前 Session 没有挂载 CSS');await patchCurrent({customCssEnabled:true},true);applyCustomCss();if(settingsTab==='appearance')renderAppearanceTab(qs('settings-content'))}
async function duplicateCssPreset(id){const p=cssPresets.find(x=>x.id===id);if(!p)return;const res=await fetchStory('/css-presets','POST',{name:`${p.name} · 副本`,css:p.css,scope:p.scope});if(res.success){await loadAll();openSettings('appearance');toast('CSS Preset 已复制')}else toast(res.msg||'复制失败')}
async function deleteCssPreset(id){if(!confirm('删除这个 CSS Preset？挂载它的 Session 会自动取消应用。'))return;const res=await fetchStory(`/css-presets/${id}`,'DELETE');if(!res.success)return toast(res.msg||'删除失败');closeEditor();await loadAll();applyCustomCss();openSettings('appearance')}
function chooseCssImport(){qs('css-file-input').value='';qs('css-file-input').click()}
async function importCssFile(file){if(!file)return;try{const css=stripEmbeddedStyleTags(await file.text());const res=await fetchStory('/css-presets','POST',{name:file.name.replace(/\.css$/i,'')||'导入 CSS',css,scope:'story'});if(!res.success)throw new Error(res.msg||'导入失败');await loadAll();openSettings('appearance');toast(`已导入 CSS「${res.data.name}」`)}catch(e){toast(`CSS 导入失败：${e.message}`)}}
