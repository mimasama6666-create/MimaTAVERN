...
function clearLiveStream(){if(liveStreamState?.timer)clearInterval(liveStreamState.timer);if(liveStreamState?.node?.isConnected)liveStreamState.node.remove();liveStreamState=null}
...
async function triggerChat(text,actionType='send'){
    ...
    try{const res=await fetchStory(`/sessions/${currentSessionId}/chat`,'POST',{text,action:actionType,temperature:getTemp(),directorNote,targetMessageId},true,handleGenerationProgress);toggleUIState(false);if(res.aborted){toast('生成已停止，主人消息已保留。');setGenerationProgress({percent:100,label:'已停止生成',detail:'主人消息仍然保存在剧情里，可以直接重试。'});setTimeout(()=>dismissGenerationPanel(),900);clearLiveStream();await openSession(currentSessionId,false);return}if(res.success){clearLiveStream();currentSessionData=res.data;qs('story-input').value='';autoResizeInput();refreshChatBox();await refreshSessionSummaryList();refreshHeader();finishGenerationProgress()}else{clearLiveStream();showGenerationError(res);toast(`生成失败 · ${res.code||'E_UNKNOWN'}`);await openSession(currentSessionId,false)}}catch(e){clearLiveStream();toggleUIState(false);showGenerationError(e);toast(`生成失败 · ${e?.code||'E_UNKNOWN'}`)}
}
