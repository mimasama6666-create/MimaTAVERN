...
(() => {
  ...
  function switchType(type){if(!TYPES.includes(type)||generating)return;activeType=type;nextQuickMode='';draftAssistantText='';editingIndex=-1;render();}
  ...
  function setConfigOpen(open){state.ui.configOpen=!!open;saveState();}
  ...
  async function send(){
    ...
    thread().push({role:'user',content:raw,requestContent:userContent,quickMode,time:new Date().toISOString()});saveState();nextQuickMode='';generating=true;generationMode='send';regeneratingIndex=-1;editingIndex=-1;draftAssistantText='';render();
  ...
  async function regenerate(index){
    ...
    const messages=[{role:'system',content:systemPrompt(activeType)},...history];
    generating=true;generationMode='regenerate';regeneratingIndex=index;editingIndex=-1;draftAssistantText='';render();
    const cfg={...storyCfg,model,temperature:p.temperature,stream:p.stream,sendTemperature:p.sendTemperature!==false,maxTokens:Number(p.maxTokens)||0};controller=new AbortController();
    try{
